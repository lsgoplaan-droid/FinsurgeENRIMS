...
def generate_board_report(
    ...
    # Header
    story.append(Paragraph("FinsurgeENRIMS", styles["title"]))
    ...
    story.append(Paragraph(
        "CONFIDENTIAL — This report is generated by FinsurgeENRIMS for internal use only. "
        "Do not distribute outside the organization without written approval from the Chief Risk Officer.",
        styles["footer"],
    ))
    story.append(Paragraph(
        f"Report ID: BOARD-{now.strftime('%Y%m%d-%H%M%S')} &nbsp;|&nbsp; FinsurgeENRIMS v1.0 &nbsp;|&nbsp; Page 1",
        styles["footer"],
    ))
    ...
    filename = f"FinsurgeENRIMS_Board_Report_{now.strftime('%Y%m%d')}.pdf"
